...
export function BulkPriceEditor({ onClose }: { onClose: () => void }) {
  ...
  const [rows, setRows] = useState<
    { id: string; label: string; labelEn: string; category: string; billing: BillingType; priceStr: string }[]
  >([]);
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-left text-gray-400 border-b border-gray-100">
                    <th className="pb-1 font-medium w-[25%]">{t('name')} (JA)</th>
                    <th className="pb-1 font-medium w-[25%]">{t('name')} (EN)</th>
                    <th className="pb-1 font-medium w-[20%]">{t('billingType')}</th>
                    <th className="pb-1 font-medium w-[15%]">{t('unitPriceUsd')}</th>
                  </tr>
                </thead>
                <tbody>
                  {rows
